...
def recommend_books(library, member):
    recommended_books = []
    ...
    borrowed_authors = {book.author for book in member.borrowed_books}


    for book in library.books:
        if book.author in borrowed_authors and book not in member.borrowed_books:
            if book not in recommended_books:
                recommended_books.append(book)

    return recommended_books if recommended_books else None




def forgot_password(library, email, new_password):
...
def signin(library, email, password):
...
def member_menu(library, member):
    while True:
        ...
        recs = recommend_books(library,member)
        if recs:
            print("Welcome! Recommended books based on your previous book issues: \n")
            for rec in recs:
        ...
        match input("Enter your choice: "):
            ...
            case "4":
                ...
                forgot_password(library,member.email,new_password)
            case "5":
                print("Logging out...")
                return
            case _:
...
def main():
    ...
    librarian = Librarian("Alice", 40, "admin@gmail.com", "Admin@123", library)
    ...
    while running:
        ...
        if choice.lower() == "l":
            ...
            user = signin(library,email,password)
            if not user:
                continue
            ...
            new_password = input("Enter new password: ")
            forgot_password(library,email,new_password)
